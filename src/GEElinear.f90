function LinearPower_GEE(mu, beta, gamma,rho0, II, JJ, KK, &
...
!calculate variance
   !do i=1,II
    ...
    do m=1,JJ
     ...
     if ((gmu(m)*gmu(m)+alpha0*gmu(m)*(1-gmu(m)))>gmu(m)) then
        !print *, '(gmu(m)*gmu(m)+alpha0*gmu(m)*(1-gmu(m)))>gmu(m)'
       !stop
     end if

     if ((gmu(m)*gmu(m)+alpha0*gmu(m)*(1-gmu(m)))<max(2*gmu(m)-1,0d0)) then
        !print *, '(gmu(m)*gmu(m)+alpha0*gmu(m)*(1-gmu(m)))<max(0,2*gmu(m)-1)'
       !stop
     end if
    ...
    do m=1,(JJ-1)
      do k=(m+1),JJ
        if (((gmu(m)*gmu(k)+alpha1*(1/mmu(m,m))*(1/mmu(k,k))))>min(gmu(m),gmu(k))) then
           !print *, '((gmu(m)*gmu(k)+alpha1*(1/mmu(m,m))*(1/mmu(k,k))))>min(gmu(m),gmu(k))'
          !stop
        end if

        if (((gmu(m)*gmu(k)+alpha1*(1/mmu(m,m))*(1/mmu(k,k))))<max(gmu(m)+gmu(k)-1,0d0)) then
           !print *, '((gmu(m)*gmu(k)+alpha1*(1/mmu(m,m))*(1/mmu(k,k))))<max(0,gmu(m)+gmu(k)-1)'
          !stop
        end if
        if (((gmu(m)*gmu(k)+alpha2*(1/mmu(m,m))*(1/mmu(k,k))))>min(gmu(m),gmu(k))) then
           !print *, '((gmu(m)*gmu(k)+alpha2*(1/mmu(m,m))*(1/mmu(k,k))))>min(gmu(m),gmu(k))'
          !stop
        end if

        if (((gmu(m)*gmu(k)+alpha2*(1/mmu(m,m))*(1/mmu(k,k))))<max(gmu(m)+gmu(k)-1,0d0)) then
           !print *, '((gmu(m)*gmu(k)+alpha2*(1/mmu(m,m))*(1/mmu(k,k))))<max(0,gmu(m)+gmu(k)-1)'
          !stop
        end if
