function LogPower_time(mu, beta, gamma, tau2, II, JJ, KK, a, b, &
    ...
    double precision :: invVar(JJ+2,JJ+2),invVarPre(JJ+2,JJ+2)         ! inverse of variance matrix
    ...
    do i=1,II
      ...
      invVar = invVar + invVarPre
        ...
        do while (finish<1)
            ...
            call der_likelihood_time_log(mu,beta,gamma,tau2, z0, z1, X(i,:), JJ, KK, a, b, &
...
subroutine der_likelihood_time_log(mu,beta,gamma,tau2, z0, z1, XX, JJ, KK, a, b, &
                        mincomp, maxcomp, GQ, GQX, GQW, derlikelihood, prob)
    implicit none
    ! ---- arg types -----------------------
    integer :: JJ, KK
    ...
    double precision :: ebb, exx
    double precision :: fbebb
    mincomp(1)=mincomp(1)
  ...
  !  faeaa = ff*eaa
    ...
    ebb = dexp(-0.5d0*a*a/tau2)
    ff = 1.0d0
    do j=1,JJ
        ff1 =dexp(mu+beta*XX(j)+gamma(j)-a)
        ff0 = 1-ff1
        ff = ff*(ff0**z0(j))*(ff1**z1(j))
    end do
    fbebb = ff*ebb
...
end subroutine der_likelihood_time_log


function LogPower_notime(mu, beta, tau2, II, JJ, KK, a, b, GQ, GQX, GQW, X_in,typeone) result (power)
    implicit none
    ! ---- arg types -----------------------
    double precision :: mu, beta, tau2,typeone,ppp
    ...
    do i=1,II
      if(i>1 .and. ALL(X_in(i,:).EQ. X_in(i-1,:))) then
        ...
        do z00 = 0,z0(i)
            ...
            do z10 = 0,z1(i)
                z11 = z1(i) - z10
                call der_likelihood_notime_log(mu, beta, tau2,a,b, z00, z01, z10, z11, GQ, GQX, GQW,&
...
end function LogPower_notime


subroutine der_likelihood_notime_log(mu, beta, tau2, a,b, z00, z01, z10, z11, GQ, GQX, GQW,&
derlikelihood_mu, derlikelihood_beta, derlikelihood_tau2, prob)
    implicit none
    ! ---- arg types -----------------------
    double precision :: mu, beta, tau2,a,b
    ! true values of beta0, beta1, and tau2
    integer :: z00, z01, z10, z11
    integer :: GQ   ! number of GQ points
    double precision :: GQX(GQ), GQW(GQ)
    ...
    double precision :: prob,likelihoodf_numer2
    ! ---------------------------------------
    double precision :: likelihoodf_denom, likelihoodf_numer, likelihoodf_denomb2

    double precision :: ff00, ff01, ff10, ff11,ffn00,ffn01,ffn10,ffn11
    double precision :: ff, ff1, ffprob, ff0prob, ff1prob
    double precision :: exx, exx1, exx2, temp, exxm
    integer :: z0, z1
    integer :: i, k
    double precision :: x
    ...
    do i=1,GQ
        ...
        ff10 = 1-ff11
        exx = dexp(-0.5d0*x*x/tau2)
        exxm= dexp(-0.5d0*(x*x)/tau2)
        ffn01 = dexp(mu-x)
        ffn00 = 1-ffn01
        ffn11 = dexp(mu+beta-x)
        ffn10 = 1-ffn11
        ff = (ffn00**z00)*(ffn01**z01)*(ffn10**z10)*(ffn11**z11)
        likelihoodf_numer = likelihoodf_numer + GQW(i) * ff * exxm
        likelihoodf_denom = likelihoodf_denom + GQW(i) * exxm
        likelihoodf_denomb2 = likelihoodf_denomb2 + GQW(i) * x * x * exxm
        !likelihoodf_numer2 = likelihoodf_numer2 + GQW(i) * ff * x * x * exx

        ! since GQX are not at limits, we ignore the cases where ff00=0 or ff01=0 or ff10=0 or ff11=0
        ff1 = ff*(z01-z00*ffn01/ffn00+z11-z10*ffn11/ffn10)
        derlikelihood_mu = derlikelihood_mu + GQW(i) * ff1 * exxm
        ff1 = ff*(z11-z10*ffn11/ffn10)
        derlikelihood_beta = derlikelihood_beta + GQW(i) * ff1 * exxm
        ff1 = ff*x*x
        derlikelihood_tau2 = derlikelihood_tau2 + GQW(i) * ff1 * exxm
        ...
        ! to avoid overflow
        if (z00<z01) then
            ffprob = ffprob*ffn01**(z01-z00)
            do k=0,(z00-1)
                ffprob = ffprob*dble(z0-k)/dble(z00-k)*ff0prob
            end do
        else
            ffprob = ffprob*ffn00**(z00-z01)
            do k=0,(z01-1)
    ...
    if (beta>=0) then
        ...
        !end if
        if (z10==0) then
            temp = (dexp(-beta)**z01)*((1-dexp(-beta))**z00)*exx1
            derlikelihood_mu = derlikelihood_mu - temp
            derlikelihood_beta = derlikelihood_beta - temp
        end if
        derlikelihood_mu = derlikelihood_mu / likelihoodf_numer + (exx1)/likelihoodf_denom
        ...
        !end if
        if (z00==0) then
            temp = (dexp(beta)**z11)*((1-dexp(beta))**z10)*exx1
            derlikelihood_mu = derlikelihood_mu - temp
          !  derlikelihood_beta = derlikelihood_beta + temp
        end if
        derlikelihood_mu = derlikelihood_mu / likelihoodf_numer + (exx1)/likelihoodf_denom
    ...
    prob = prob / likelihoodf_denom
end subroutine der_likelihood_notime_log
